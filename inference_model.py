...
class LSHAC_NER_Prediction():
    ...
    def __init__(self,clusters:List[Tuple[int,int]],logits:List[torch.Tensor],types_list:List[str],sentence_mask:torch.Tensor, word_ids:List[int]=None):
        ...
        self.logits=torch.stack(logits).cpu()
        ...
        self.probs=torch.softmax(torch.stack(logits),dim=1).cpu()
        ...
        sorted_awc=[(c,a,conf) for (c,a,conf) in sorted(assignments_with_confidence,key=lambda x: (x[2],x[0][1]-x[0][0]),reverse=True)]
    ...
    def __repr__(self):
        return f"LSHAC_Prediction(assignments={self.assignments},types_list={self.types_list},seq_length={self.seq_length},sentence_mask={self.sentence_mask})"
    ...
    def get_token_assignments(self, flat:bool=False)->List[Tuple[Tuple[int,int],int]]:
        return self.assignments if not flat else self.flat_assignments
    
    def get_word_assignments(self, flat:bool=False) -> List[Tuple[Tuple[int,int],int]]:
        """
        assignments adjusted to word spans
        """
        assignments=self.get_token_assignments(flat=flat)
        if len(assignments)==0:
            return []
        assignment_clusters,assignment_types=zip(*assignments)
    ...
    def get_pydot_tree(self, colors:Dict[str,str]={}):
        ...
        for a in sorted(assign,key=lambda t: (t[0][0],t[0][1]-t[0][0])):
            ...
            label=f"{self.types_list[a[1]]} ({confidence:.4f})" if confidence>0 else f"{self.types_list[a[1]]}"
            node=pydot.Node(str(a[0]),label=label, style="filled", fillcolor=color)
            G.add_node(node)
        already_added=[]
