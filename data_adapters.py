...
def convert_art_dataset(source_dataset: DatasetDict) -> DatasetDict:
    """
    Each element is of the following format:
    {'text': '26 still-life with a delft vase of carnations and other flowers,\noil on canvas, signed with initials h.e.b., unframed, 13in.',
 'tokens': ['26',
  'still',
  '-',
  'life',
  ...
  'in',
  '.'],
 'entities': [{'start_token': 15, 'end_token': 18, 'label': 'TITLE'}]}
    """
    target_dataset=DatasetDict()
    entity_types=["O"]
    all_entities=source_dataset["train"]["entities"]
    ...
    def map_entity(all_entities:List[List[dict]]):
        ...
        for entities in all_entities:
            spans=[]
            for entity in entities:
                spans.append({"start":entity["start_token"], "end":entity["end_token"], "label":class_label_obj.str2int(entity["label"])})
            all_spans.append(spans)
        return all_spans
    def map_batch(batch):
        return {"id": batch["id"], "tokens": batch["tokens"], "spans": map_entity(batch["entities"])}
